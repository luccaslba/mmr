...
class ConfigRole(Cog):
    def __init__(self, bot):
        self.bot = bot

    @app_commands.command(description="Configurar o cargo")
    async def config_role(self, interact: discord.Interaction):
        ...
        if guild:
            if interact.user.get_role(guild.perm_cmd_role_id) or interact.user.id == config_bot.OWNER_ID:
                ...
                await interact.edit_original_response(embed=sucess, view=AddRoleView())
